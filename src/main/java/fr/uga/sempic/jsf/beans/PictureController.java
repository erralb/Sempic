...
//		rdfResources = rdfs.getPhotoModel(current.getId()).toString();
    ...
    public String create() {
        try {
            //Save File
            // inspired by https://stackoverflow.com/questions/18664579/recommended-way-to-save-uploaded-files-in-a-servlet-application
            ...
            int i = 0;
            while(Files.exists(newFilePath, new LinkOption[]{LinkOption.NOFOLLOW_LINKS})) {
                i++;
                File newName = new File(uploads, String.valueOf(i) + file.getSubmittedFileName());
                newFile.renameTo(newName);
                newFilePath = newFile.toPath();
            }
            // do the copy
            try (InputStream input = file.getInputStream()) {
                Files.copy(input, newFilePath);
            }
            
            //Save Picture Model
            current.setUser(auth.currentUser());//set logged user as Album owner
            current.setAdded(new Timestamp(System.currentTimeMillis()));
            current.setFilename(String.valueOf(i) + file.getSubmittedFileName());
